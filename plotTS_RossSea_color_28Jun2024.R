...
# load NBP1201 CTD Data from the .mat file
NBP1201_ctd <- readMat("nbp1201_ctd.mat")

# load the Ross Sea Climatology CTD data set
rossclim <- readMat("rossclimII_update.mat")
# Create an oce CTD data structure 
climsal <- as.vector(rossclim$gsal)
climtem <- as.vector(rossclim$gtem)
climprs <- as.vector(rossclim$gprs)
climlat <- as.vector(rossclim$latg.array)
climlat <- rep(climlat, 40) # need to repeat this 40 times for the 40 levels that are in the other variables above
climlon <- as.vector(rossclim$long.array)
climlon <- rep(climlon, 40) # need to repeat this 40 times for the 40 levels that are in the other variables above
climtmp <- as.ctd(salinity = climsal, temperature = climtem, pressure = climprs, latitude = climlat, longitude = climlon)
...
# Commented out because I have already done this and then cropped the image
#png("TS_RB_Leg1_color.png", width = 4800, height = 4800, res = 600)
#cm1 <- colormap(RB_37_44[["pressure"]])
#drawPalette(colormap = cm1, zlab = "Depth [m]", cex = 1.25, pos = 1)
#dev.off()

# make a plot for Leg 1
png("TS_RB_Leg1_color_with_clim.png", width = 4800, height = 4800, res = 600)
cm1 <- colormap(RB_37_44[["pressure"]])
plotTS(climtmp, pch = 19, cex = 0.5, cex.axis = 1.25, cex.lab = 1.25, cex.rho = 1, eos="gsw", mar = par("mar"), col = "grey")
plotTS(RB_37_44, pch = 19, cex = 0.75, cex.axis = 1.25, cex.lab = 1.25, cex.rho = 1, col = cm1$zcol, eos="gsw", mar = par("mar"), add = TRUE)
# Panel label
text(x = 33.6, y = 2.2, labels = "A", col = "black", cex = 1.5)
dev.off()

# make a plot for Leg 2
png("TS_RB_Leg2_color_with_clim.png", width = 4800, height = 4800, res = 600)
plotTS(climtmp, pch = 19, cex = 0.5, cex.axis = 1.25, cex.lab = 1.25, cex.rho = 1, eos="gsw", mar = par("mar"), col = "grey")
plotTS(RB_74_80, pch = 19, cex = 0.75, cex.axis = 1.25, cex.lab = 1.25, cex.rho = 1, col = cm1$zcol, eos="gsw", mar = par("mar"), add = TRUE)
# Water mass labels
text(x = 34.2, y = -1, labels = "AASW", col = "black", cex = 1.5)
text(x = 34.72, y = 0.2, labels = "MCDW", col = "black", cex = 1.5)
text(x = 34.83, y = 1.3, labels = "CDW", col = "black", cex = 1.5)
text(x = 34.8, y = -1.3, labels = "MSW", col = "black", cex = 1.5)
text(x = 34.9, y = -2.2, labels = "SW", col = "black", cex = 1.5)
# Panel label
text(x = 33.6, y = 2.2, labels = "B", col = "black", cex = 1.5)
dev.off()
