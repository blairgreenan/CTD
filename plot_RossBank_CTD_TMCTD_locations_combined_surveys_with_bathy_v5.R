...
# Convert depth from negative to positive
elevation_long <- -1 * elevation_long
# Create data frame
...
#  geom_raster(aes(fill = Depth), interpolate = TRUE) + 
  geom_contour_filled(breaks = c(150, 250, 350, 450, 550, 650, 750)) + 
...
#  scale_fill_cmocean(name = "deep", direction = -1) +
  ...
  labs(fill = "Depth (m)") +
  scale_y_continuous(limits=c(-77.05,-76.3), breaks=c(-77.0, -76.8, -76.6, -76.4), labels=c("77.0\u00b0S", "76.8\u00b0S", "76.6\u00b0S", "76.4\u00b0S"))  + 
  scale_x_continuous(limits=c(176.8,180.5), breaks=c(177, 178, 179, 180), labels=c("177\u00b0E", "178\u00b0E", "179\u00b0E", "180\u00b0")) +
#  scale_x_continuous(limits=c(176.8,180.5), breaks=c(177, 178, 179, 180), labels=c("177\u00b0E", "178\u00b0E", "179\u00b0E", "180\u00b0"), position = "top") +
...
#ggsave("RossBank_CTD_Surveys_ggsave.png", width = 10, height = 8, units = c("cm"), dpi = 1200, bg = "white", scale = 1.75)
